...
WORKDIR /src
COPY ["MemcachedRecipes.StoreValuesLambda.csproj", "MemcachedRecipes.StoreValuesLambda/"]
RUN dotnet restore "MemcachedRecipes.StoreValuesLambda/MemcachedRecipes.StoreValuesLambda.csproj"

WORKDIR "/src/MemcachedRecipes.StoreValuesLambda"
COPY . .
RUN dotnet build "MemcachedRecipes.StoreValuesLambda.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "MemcachedRecipes.StoreValuesLambda.csproj" \
...
Execute unit tests
```
cd "MemcachedRecipes.StoreValuesLambda/test/MemcachedRecipes.StoreValuesLambda.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "MemcachedRecipes.StoreValuesLambda/src/MemcachedRecipes.StoreValuesLambda"
