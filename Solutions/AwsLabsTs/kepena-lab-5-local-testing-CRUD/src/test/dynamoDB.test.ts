...
it('deleteItem(id) - deleting the brand new created/updated record ', async() => {
    const spy = jest.spyOn(AWS.DynamoDB.DocumentClient.prototype, 'delete');
    spy.mockImplementation(()=>{
        return{
            promise(){
                return {};
            },
        } as any as AWS.Request<AWS.DynamoDB.DocumentClient.DeleteItemOutput, AWS.AWSError>;
    });
    const result = await dynamodb.deleteItem(idTestRecordUpdate+"+updateNewRecord", tablename);
    expect(result).toMatchObject({});
    spy.mockRestore();
});
...
it('scan(tablename) - returns all the records from the table ', async() => {
    const spy = jest.spyOn(AWS.DynamoDB.DocumentClient.prototype, 'scan');
    spy.mockImplementation(() => {
    return {
        promise () {
            return {
    ...
    } as any as AWS.Request <AWS.DynamoDB.DocumentClient.ScanOutput, AWS.AWSError>;
    });
...
it('error query() - Should return an error: missing paramenters like username', async() => {
    expect.assertions(1);
    try {
        const spy = jest.spyOn(AWS.DynamoDB.DocumentClient.prototype, 'query');
        spy.mockImplementation(() => {
...
it('error scan() - Should return an error: missing paramenters like tablename', async() => {
    expect.assertions(1);
    try {
        const spy = jest.spyOn(AWS.DynamoDB.DocumentClient.prototype, 'scan');
        spy.mockImplementation(() => {
            throw new Error("MissingRequiredParameter: Missing required key 'TableName' in params");
        }); 
        await dynamodb.scan();
        spy.mockRestore();
    } catch (e) {
        expect(e.message).toBe("Error: MissingRequiredParameter: Missing required key 'TableName' in params");

    }
});

it('error deleteItem() - Should return an error: missing paramenter(s) like tablename', async() => {
    expect.assertions(1);
    try {
        const spy = jest.spyOn(AWS.DynamoDB.DocumentClient.prototype, 'delete');
        spy.mockImplementation(() => {
        ...
        expect(e.message).toBe("Error: Internal Server Error");
    }
});